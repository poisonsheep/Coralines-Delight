...
import static io.github.poisonsheep.coralinesdelight.resources.bbmodel.BBTexture.MISSING;
    ...
    public BBModel(JsonObject model, ResourceLocation identifier) {
        ...
        model.get("elements").getAsJsonArray().forEach(element -> {
            ...
            if (type.equals("cube")) {
                ...
                CoralinesDelight.LOGGER.warn("Unknown object type {}", type);
            }
        });
        ...
        model.get("outliner").getAsJsonArray().forEach(element -> {
            if (element.isJsonPrimitive()) {
                ...
                if (object != null) {
                    ...
                    CoralinesDelight.LOGGER.warn("Object with uuid {} not found", element.getAsString());
                }
            } else {
